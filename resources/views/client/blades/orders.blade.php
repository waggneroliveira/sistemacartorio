...
<script>
    ...
    function formatMoney(value) {
        ...
        if (isNaN(num) || (!num && num !== 0)) return 'R$ 0,00';
        ...
        let filtered = requests.filter(request => {
            ...
            if (currentSearch) {
                ...
                return request.protocolo.toLowerCase().includes(searchLower) ||
                    ...
                    (request.descricao && request.descricao.toLowerCase().includes(searchLower));
            }
            return true;
        });
        ...
        container.innerHTML = filtered.map(request => `
            <div class="order-card p-3 mb-2 ${selectedOrderId === request.id ? 'selected' : ''}" 
                ...
                <div class="d-flex justify-content-between align-items-center mt-2">
                    <small class="text-muted"><i class="bi bi-calendar3"></i> ${request.dataSolicitacao}</small>
                    <small class="text-success">${request.status === 'awaiting_payment' || request.status === 'aguardando_pagamento' ? formatMoney(request.valor) : 'Ver detalhes →'}</small>
                </div>
            </div>
        `).join('');
    ...
    function updateStats() {
        ...
        const aguardandoPagamento = requests.filter(r => r.status === 'awaiting_payment' || r.status === 'aguardando_pagamento').length;
                ...
                <div class="payment-card">
                    <div class="row align-items-center">
                        <div class="col-md-6">
                            ...
                            <p class="text-muted small mb-0">${request.descricao ? request.descricao.substring(0, 100) : ''}</p>
                        </div>
    ...
    // Função confirmarPagamento - Atualiza para payment_approved
    function confirmarPagamento(requestId, metodo, valor) {
        const request = requests.find(r => r.id === requestId);
        if (!request) return;
        
        // Atualiza status para payment_approved
        request.status = 'payment_approved';
        request.statusTexto = 'Pagamento aprovado';
            ...
            <div class="fade-in">
                <div class="d-flex justify-content-between align-items-start mb-3">
                    <div>
                        ...
                        <p class="text-muted small"><i class="bi bi-calendar"></i> Solicitado em: ${request.dataSolicitacao}</p>
                    </div>
                ...
                <!-- SÓ MOSTRA O BOTÃO DE PAGAMENTO SE O STATUS FOR awaiting_payment -->
