        ...
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box">
                        ...
                        <h4 class="page-title">Editar Status</h4>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-lg-8">
                    <div class="card">
                        <div class="card-body">
                            <form action="{{ route('admin.dashboard.requestStatus.update', $requestStatus) }}" method="POST">
                                @csrf
                                @method('PUT')
                                ...
                                <div class="mb-3">
                                    ...
                                    @enderror
                                    <small class="text-muted">Identificador não pode ser alterado</small>
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="input-group">
                                        <input type="color" name="color" id="color" class="form-control form-control-color @error('color') is-invalid @enderror" 
                                            value="{{ old('color', $requestStatus->color) }}" 
                                            style="width: 80px;">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="number" name="order" id="order" class="form-control @error('order') is-invalid @enderror" 
                                        value="{{ old('order', $requestStatus->order) }}" 
                                        min="0" placeholder="0">
                                    @error('order')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="mb-3">
                                    <div class="form-check">
                                        <input type="checkbox" name="is_active" id="is_active" class="form-check-input" 
                                            value="1" {{ old('is_active', $requestStatus->is_active) ? 'checked' : '' }}>
                                ...
                                <div class="mb-3">
                                    <div class="form-check">
                                        <input type="checkbox" name="is_default" id="is_default" class="form-check-input" 
                                            value="1" {{ old('is_default', $requestStatus->is_default) ? 'checked' : '' }}>
                                ...
                                <div class="mb-3">
                                    <div class="form-check">
                                        <input type="checkbox" name="is_final" id="is_final" class="form-check-input" 
                                            value="1" {{ old('is_final', $requestStatus->is_final) ? 'checked' : '' }}>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="mb-3">
                                ...
                                <div style="height: 50px; background-color: {{ old('color', $requestStatus->color) }}; border-radius: 4px; border: 1px solid #ddd;"></div>
                            </div>

                            <div class="mb-3">
                                <label class="text-muted small">Badge</label>
                                <span class="badge" style="background-color: {{ old('color', $requestStatus->color) }}; padding: 8px 12px; font-size: 14px;">
                                    {{ old('label', $requestStatus->label) }}
                                </span>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <ul class="small text-muted">
                                    <li><strong>Nome:</strong> {{ $requestStatus->name }}</li>
                                    <li><strong>Padrão:</strong> {{ $requestStatus->is_default ? 'Sim' : 'Não' }}</li>
                                    <li><strong>Final:</strong> {{ $requestStatus->is_final ? 'Sim' : 'Não' }}</li>
                                    <li><strong>Ativo:</strong> {{ $requestStatus->is_active ? 'Sim' : 'Não' }}</li>
                                    <li><strong>Ordem:</strong> {{ $requestStatus->order }}</li>
                                </ul>
                            </div>

                            @if($requestStatus->requests()->count() > 0)
                                <div class="alert alert-info small">
                                    <i class="bi bi-info-circle"></i> Este status está sendo usado em {{ $requestStatus->requests()->count() }} solicitação(ões)
                                </div>
                            @endif
                        </div>
                    </div>
